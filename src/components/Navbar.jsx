export const Navbar = () => {
    ...
    return (
        <header className="w-full bg-black flex text-white">
            <div className="lg:px-32 px-6 py-4 w-full">
                <nav className="flex items-center justify-between relative w-full">
                    ...
                    {/* Right: Invisible placeholder for symmetry */}
                    {/* This div needs to match the width of the left ul for perfect centering if you use justify-between 
                    <div className="flex-1 flex justify-end">
                        <ul className="flex gap-x-2 invisible">
                            {navitems.map((item, key) => (
                                <li className="" key={key}> <a href={item.href}>{item.text}</a></li>
                            ))}
                        </ul>
                    </div>*/}
                    <ul className="flex gap-x-12  tracking-[0.8px] montserrat-regular items-center">
